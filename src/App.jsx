...
const App = () => {
  ...
  //framer-motion k liye useState
  const [showButton, setShowButton] = useState(false);

   useEffect(() => {
    const fetchGeoJSON = async () => {
      try {
        const res = await fetch("https://aymnsk.github.io/geoai-frontend/data/zones.geojson");
  ...
  const handleAsk = async () => {
    ...
    try {
        const res = await fetch("https://b6fb71eb-1f14-4fe4-a8ff-750b06611f40-00-312lb97pq5f33.sisko.replit.dev/ask", {
            method: "POST",
        ...
        if (match && geoData) {
            ...
            const foundFeature = geoData.features.find((f) => f.properties.name === zoneName);
            if (foundFeature) {
                const [lng, lat] = foundFeature.geometry.coordinates;
                setHighlightCoords({ lat, lng, zoneName });
            }
        }
    } catch (err) {
        console.error("❌ Backend error:", err);
        setResult("❌ Could not connect to GeoAI backend.");
    }
};
  ...
  return (
    <div className='main w-full flex flex-col items-center justify-center'>
      ...
      {/* 1 */}
   <h1 className="title forum-regular cursor typewriter-animation text-center mt-20 mb-5 px-4 text-xl sm:text-3xl md:text-4xl lg:text-6xl leading-snug break-words">
  GeoAI — Smart Zone Recommender
</h1>

    {/* 2 */}
    <div className="w-full max-w-[80%] mt-8 sm:max-w-[90%] md:max-w-[80%] lg:max-w-[90%] mx-auto h-[300px] lg:h-[600px] rounded-xl overflow-hidden">
  <GeoMap highlightCoords={highlightCoords} geoData={geoData} />
</div>
     ...
     {/* 3 */}
   <div className = "input-box1 flex flex-col items-center justify-center space-y-4  w-full px-4 mb-6">
 
   
 <div className="search-container w-full md:w-1/2 flex items-center justify-center relative mx-auto ">
  <input type="text" className="search-input w-full pl-10 pr-4 min-h-24 rounded-md " placeholder="Search your question..." 
  ...
  <div className="search-icon">
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="sp flex justify-center w-full md:w-auto">

  <motion.button className="sparkle-button  px-6 my-8 " onClick={() => {handleAsk(); setShowButton(false);}} disabled={result === "🧠 Thinking..."} 
  initial={false} animate={showButton ? { x: 0 } : { x: "500vh" }}  transition={{type: "spring", stiffness: 200}}>
  ...
  <span aria-hidden="true" class="particle-pen">
    ...
    <svg class="particle" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M6.937 3.846L7.75 1L8.563 3.846C8.77313 4.58114 9.1671 5.25062 9.70774 5.79126C10.2484 6.3319 10.9179 6.72587 11.653 6.936L14.5 7.75L11.654 8.563C10.9189 8.77313 10.2494 9.1671 9.70874 9.70774C9.1681 10.2484 8.77413 10.9179 8.564 11.653L7.75 14.5L6.937 11.654C6.72687 10.9189 6.3329 10.2494 5.79226 9.70874C5.25162 9.1681 4.58214 8.77413 3.847 8.564L1 7.75L3.846 6.937C4.58114 6.72687 5.25062 6.3329 5.79126 5.79226C6.3319 5.25162 6.72587 4.58214 6.936 3.847L6.937 3.846Z" fill="black" stroke="black" stroke-linecap="round" stroke-linejoin="round"></path>
         ...
         {/* 4 */}
     <div className="search-container h-48 mb-40" id="result">
  {result ? result : <span className="text-gray-400">Answers will appear here</span>}
</div>
